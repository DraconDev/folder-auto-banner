...
pub fn run_uninstall_hook() -> Result<()> {
    ...
    let hook_start = "# cfm auto-banner hook";
    let hook_end = match shell.as_str() {
        "zsh" => "_cfm_hook  # fire on new shell/tab startup",
        "bash" => "PROMPT_COMMAND=\"_cfm_hook${PROMPT_COMMAND:+;$PROMPT_COMMAND}\"",
        _ => "_cfm_hook  # fire on new shell/tab startup",
    };

    if !content.contains(hook_start) {
        println!("ℹ️  No cfm hook found in {}", config_path.display());
        return Ok(());
    }
    ...
    if found_start {
        ...
        println!("✅ Removed cfm hook from {}", config_path.display());
